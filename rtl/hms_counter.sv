...
module hms_counter #(
    ...
    always_ff @(posedge clk) begin
        if(enable) seconds <= next_second;
    end

    always_comb begin
        if(enable && (seconds > MaxSeconds)) begin
            second_rollover = 1'd1;
        end

        if(enable && (minutes > MaxMinutes)) begin
            minute_rollover = 1'd1;
        end
    end

    up_down_counter u_hour(
        .clk(clk),
        .enable(minute_rollover),
        .up(1'b1),
        .count(hours)
    );
    up_down_counter u_minute(
        .clk(clk),
        .enable(second_rollover),
        .up(1'b1),
        .count(minutes)
    );
    up_down_counter u_second(
        .clk(clk),
        .enable(1'b1),
        .up(1'b1),
        .count(seconds)
